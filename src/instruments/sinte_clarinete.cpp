...
Sinte_Clarinete::Sinte_Clarinete(const std::string &param)
...
void Sinte_Clarinete::command(long cmd, long note, long vel) {
...
const vector<float> & Sinte_Clarinete::synthesize() {
  if (not adsr.active()) {
    x.assign(x.size(), 0);
    bActive = false;
    return x;
